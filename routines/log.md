...
4. Food `source_date` > `totals_date`: sum its Foods, write the Meal's seven totals, `totals_date`, `estimated` first; cooked g × `weight_g`/`cooked_weight_g`, else guess shrink, say the error, `~`; macros whole, rest one decimal, half up.
