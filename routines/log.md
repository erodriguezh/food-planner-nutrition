...
## Steps
1. Today, else past date: its Day; closed: say so; older open: auto-close.
...
3. Slot: word, clock <11 breakfast, <15 lunch, <18 snack, dinner; filled earlier: next in order.
4. Meal by `portion` or g. Food `source_date` > `totals_date`: sum its Foods, write the Meal's seven totals, `totals_date`, `estimated` first; cooked g × `weight_g`/`cooked_weight_g`, else guess shrink, say the error, `~`; whole macros, rest one decimal, half up.
...
6. Rewrite Day totals, `estimated`; never the Pantry, over: "was that the last of X?".
