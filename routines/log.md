...
## Steps
1. Today unless named; past date: its Day; closed Day: `routines/close-day.md` refresh, say so; older open: auto-close.
...
4. Meal: portion or g; Food `source_date` > `totals_date`: sum its Foods, write the Meal's seven totals, `totals_date`, `estimated` first; cooked g × `weight_g`/`cooked_weight_g`, else guess shrink, say the error, `~`; macros whole, rest one decimal, half up.
...
6. Rewrite Day totals, `estimated`; never the Pantry, over its amount: "was that the last of X?".

## Write
New Day `status: open`, `goal: "[[Goals]]"`, `state.md` `open_day`, month line; commit `log: <date> <slot> <name> <amount>`.

## Reply
`routines/rebalance.md`: slot, `~`.
