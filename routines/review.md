...
3. Average per day = Day totals ÷ days counted, rounded by `routines/log.md` step 4; `~` when a counted Day is `estimated: true`.
4. Days on target: verdicts reading `on target`. Most common miss: the most frequent `<macro> low|high`, with its day count; a tie names each.
5. No Day counted: `Average: n/a`, `On target: 0 of 0`, `Most common miss: none`.
