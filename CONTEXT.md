...
- **Refresh**: the mode of close the day that a log into a closed or auto-closed Day triggers. The Summary is rebuilt from the corrected totals with the goal line it already carries; the status and the State stay; it rides on the `log:` commit and the agent says the Day was corrected.
...
- **Acceptance run**: the scripted seeded day in `acceptance/seeded_day.py`: ten turns of the prototype conversation played by a stand-in for the agent against the built vault, with the files asserted after every turn and the lint run after every commit. It tests the implementation of the current commit and its own seed fixture, never the live mutable data, so its result is the same on top of any valid vault state. Proves the routines on real files; stores nothing.
- **Seed fixture**: the canonical mutable data of an acceptance run: the Goals, the Pantry, the Meal, the Foods, `index.md` and a `state.md` with no open Day. It is written on the throwaway branch before the first turn as one setup commit `acceptance: seed fixture`, lint-green like every other commit and never one of the routine-step commits. Every number the run asserts comes from it.
- **Throwaway branch**: the branch `acceptance/seeded-day-<stamp>` an acceptance run makes in its own worktree and deletes at the end. It carries the seed fixture and the run's own commits; its nodes and Days never land on `main`, and the live vault never changes.
...
- **Lint**: the script `lint/vault_lint.py` that checks every node, the Index, the State, the Router, the Context MCP wiring (the skill file and its one Router pointer), `AGENTS.md` and the routines against the schemas. Runs locally, by hand or from the pre-commit hook in `.githooks/`, and in the GitHub Action `.github/workflows/lint.yml` on every push to `main`. No model involved.
