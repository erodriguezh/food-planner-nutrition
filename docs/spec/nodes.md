...
The weekly review (`routines/review.md`, ticket #26) is computed in chat from the Day files of one calendar week, Monday to Sunday, and the Goals node. It writes nothing, so the lint has nothing to check; the routine text is the contract. It counts `closed` and `auto-closed` Days only, states the missing days and the auto-closed count, leaves the week's open Day out with one line naming its date, gives the average per day against the target on two lines, the days whose verdict reads `on target`, and the most common `<macro> low|high` of the verdicts with its day count. The average carries the `~` when any counted Day is estimated. The whole reply is under ten lines.
